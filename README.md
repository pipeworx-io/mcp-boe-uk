...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/boe_uk_get_series \
  -H 'Content-Type: application/json' \
  -d '{"series_codes":["IUDBEDR"],"from":"2024-01-01"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/boe_uk_get_series`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
